defmodule ExsemanticaApi.RateLimitCache do
    ...
    case :ets.lookup(state, idx) do
      ...
      [{_idx, old, multi}] ->
        ...
        cond do
          # expired
          reset < unix ->
            :ets.insert(state, {idx, (new = %{old | remaining: limit - 1, reset: unix + interval}), 1})
            {:reply, {:ok, new}, state}
  ...
  @spec fire(any, non_neg_integer, non_neg_integer) :: {:ok, any} | {:error, {:rate_limited, non_neg_integer} | :forbidden}
  @doc """
