defmodule ExsemanticaApi.Users do
  require Logger
  ...
  def handle_use(_agent, context, _idx, %{:qmax => qmax} = extra) do
    ...
    query_valid = ExsemanticaPhx.Sanitize.valid_username?(query)
       ...
       "GET" ->
         cond do
           qmax > 0 and query_valid ->
             result =
               ExsemanticaPhx.Search.users("%" <> query <> "%", [limit: qmax], :query)
               |> Enum.map(&to_json/1)
           ...
           query_valid ->
             result = ExsemanticaPhx.Search.users("%" <> query <> "%", [], :count)
  ...
  def to_json([user, biography, registered]) do
    %{
      user: user,
